...
THIS IS GENERATED CODE.  DO NOT CHANGE THIS SPECIFICATION.

Use streamlit to create a responsive (mobile and desktop) and attractive web interface for the Chatty Friend application.  This server should run on a mac or a pi.

The website will write two files: chatty_config.json and chatty_secrets.json.

the configuration web server should run on a Raspberry Pi 4 running ubuntu

The web server has three stages:
1) connectivity:  skip this stage if running on a mac.  on a pi, during this stage, all we want to do is get the pi on the correct wifi network.  
The configured network is stored in chatty_config.json in the WIFI_SSID and WIFI_PASSWORD fields.  
if there is no network configured, look for a known connection in the WIFI_KNOWN_CONNECTION dictionary in chatty_config.json and if found, restore it to the WIFI_SSID and WIFI_PASSWORD fields.
if there is a network configured in WIFI_SSID and WIFI_PASSWORD, attempt to connect to it.  If it will connect, save it to WIFI_KNOWN_CONNECTION and exit connectivity mode. If it will not connect, enter hotspot mode.  
In hotspot mode, the only thing the web UI shows is SSID and Password fields, a "Save Wifi" button, and a "Cancel" button.  make sure fields don't capitalize automatically and passoword is not displayed
unless user enables display.  Make sure there's a note indicating that the pi requires 2.4Ghz wifi.  while in hotspot mode, every 60 seconds the web server should use espeak to say "waiting for wifi connection"

2) authentication: skip this stage if running on a mac.  everhthing else here is for the pi.... once connected to the configured wifi network, the web server should display a password prompt with a hint.  the password and hint are stored in chatty_secrets.json in the assistant_password and assistant_password_hint fields.
the web server should go no further until the user authenticates.  IF there is no configuration activity for 10 minutes (saved in LAST_CONFIG_EDIT_TIME chatty_config.json), the web server should automatically log out the user and go back to connectivity mode.

3) configuration: The configuration page should be responsive and attractive for use on mobile and desktop.

config sections:
- have a "RESET TO DEFAULT" button that will reset the config to the default values.  make the user type "reset" into a text field to confirm that they want to do this.  It should remove all keys except the ones related to WIFI.
you can "from chatty_config import default_config" to get the default values.
- Profile - the user profile includes:
    "USER_NAME" : string
    "change password" button.  when pressed, show a modal that requires two identical password inputs.  make sure it does not autocapitalize.  also make sure there is a hint input.  reject with a nice error if the passwords don't match.
    Also include a pop-up modal that allows the user to see the USER_PROFILE (stored as an array of strings) as a list of editable paragraphs.  Provide the option to delete or edit any paragraph. also let them add new entries or delete all at once.
    Also include a pop-up modal that allows the user to see the PRIOR_PRE_ESCALATION_NOTES (stored as an array of strings) as a list of editable paragraphs.  Provide the option to delete or edit any paragraph. also let them add new entries or delete all at once.
    Also include a drop-down to choose a time zone.  if the user selects a time zone, save it to TIME_ZONE in chatty_config.json.
    display CONTACTS (saved as a list of dictionaries with name, type [primary or other], email, and phone) as a list of editable contacts.  Provide the option to delete or edit any contact property. also let them add new contacts or delete all at once.  Validate that names are not blank, emails are something@something.something, phone numbers either include + and country code or else fit US standards (so add a +1 automaticaly before saving)
    MAX_PROFILE_ENTRIES : editable integer, minimum of 10 max of 10,000.
    SECONDS_TO_WAIT_FOR_MORE_VOICE : after this many seconds of silence, the user is assumed to be done talking.  The default is 1.0.
    ASSISTANT_EAGERNESS_TO_REPLY : provide a note indicating that this value should be between 0 (assistant will be very eager to reply) to 100 (assistant will give the user more time to reply)
    "VOICE" : assistant voice, must be one of the choices in the VOICE_CHOICES list.
    "SPEED" : include a note idicating that a lower value here will make the assistant speak more slowly, 0 to 100.
    "VOLUME" : include a note indicating that a lower value here will make the assistant speak more quietly, 0 to 100.
    "AUTO_GO_TO_SLEEP_TIME_SECONDS" : indciate that this is the number of seconds before the assistant will automatically go to sleep if ther eis no activity.

- supervisor profile - include a note indicating that if a contact is setup as "primary" they have the option to receive activity summaries
    "AUTO_SUMMARIZE_EVERY_N_MESSAGES" : indicate that all chats will be summarized when finished but this value allows intermediate summaries.  The default is 20.
    "SUPERVISOR_INSTRUCTIONS": free form text block entry.  inform the user that this is instructions for the supervisor to describe important things to watch out for.

- Network - on pi only, include a "Change Wifi" button.  allow user to enter wifi SSID and password on a pop-up modal.  make sure it does not autocapitalize. reject blank.  if both fields entered non-blank, save them to WIFI_SSID and WIFI_PASSWORD in chatty_config.json and restart (pi only).

- AI - include a "Change AI Model" button that edits three text fields and a list of voice choices.  reject blanks.  Warn user not to edit if they don't know what they're doing.
    "REALTIME_MODEL" : string 
    "AUDIO_TRANSCRIPTION_MODEL" : string
    "WS_URL" : string
    "VOICE_ASSISTANT_SYSTEM_PROMPT" : free form text block entry.

- Personality - include a button that opens a modal with a large text area for the system prompt.   Warn user not to edit if they don't know what they're doing.
Also include a drop-down to choose a voice, set the volume (0 to 100) and speed (0 to 100).

- Content - include a button that opens a modal and allows configuration of the news provider (see tools/news_service.py->RSS_NEWS_FEEDS) by selecting the provider name
    "NEWS_PROVIDER" : string, must be one of the keys in the RSS_NEWS_FEEDS dictionary in tools/news_service.py

- Secrets - include an "update secrets" button that opens a modal with a large text area.  Let the user know not to do this unless they know what they're doing.
Ask them to paste json-formatted secrets into the text area. 
if the json is valid, save a copy of the existing secrets.json file as chatty_secrets.json.bak, then merge the contents into the existing chatty_secrets.json file.
Provide a button to revert to the original secrets.json file.  require the user to type "revert" into a text field to confirm.
...
provide robust error checking on all fields.  Provide nice messages to help the user recover from errors.  Never save bad values to config.
